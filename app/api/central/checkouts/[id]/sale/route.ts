...
export async function POST(
  ...
  let body: ManualSaleInput;

  try {
    body =
      (await request.json()) as ManualSaleInput;
  } catch {
    return NextResponse.json(
      {
        error:
  ...
  try {
    ...
    /*
     * Venda manual também representa
     * uma conversão real daquele lead.
     *
     * Não preenchemos os campos Kiwify.
     */
    await client.query(
      `
        UPDATE public.leads_precheckout
        SET
          pago = TRUE,
    ...
    console.error(
      "Erro ao registrar venda manual:",
      error,
    );
